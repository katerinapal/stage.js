...
import { awaitjs as utilawait_awaitjsjs } from "./util/await";
var DEBUG;
if (typeof DEBUG === 'undefined')
  DEBUG = true;

utilstats_statsjsjs.create = 0;

function Class(arg) {
  if (!(this instanceof Class)) {
    if (utilis_isjs.fn(arg)) {
      return Class.app.apply(Class, arguments);
    } else if (utilis_isjs.object(arg)) {
...
Class.config = function() {
  if (arguments.length === 1 && utilis_isjs.string(arguments[0])) {
    return _config[arguments[0]];
  }
  if (arguments.length === 1 && utilis_isjs.object(arguments[0])) {
    utilextend_extendjsjs(_config, arguments[0]);
  }
  if (arguments.length === 2 && utilis_isjs.string(arguments[0])) {
...
var loading = utilawait_awaitjsjs();
