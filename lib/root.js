...
Root._super = ClassBinding;
Root.prototype = (0, _create.createjs)(Root._super.prototype);

ClassBinding.root = function (request, render) {
...
function Root(request, render) {
  ...
  var loop = function loop(now) {
    ...
    statsBinding.tick = statsBinding.node = statsBinding.draw = 0;
    ...
    statsBinding.fps = elapsed ? 1000 / elapsed : 0;
  };
...
Root.prototype.viewport = function (width, height, ratio) {
  if (typeof width === 'undefined') {
    return (0, _extend.extendjs)({}, this._viewport);
  }
  this._viewport = {
    width: width,
    height: height,
  ...
  this.viewbox();
  var data = (0, _extend.extendjs)({}, this._viewport);
  this.visit({
