...
Class.atlas = function(def) {
  var atlas = is_fn.fn(def.draw) ? def : new Atlas(def);
  if (def.name) {
  ...
  if (is_string.string(def.image)) {
    url = def.image;
  } else if (is_hash.hash(def.image)) {
...
Atlas.prototype = create.createjs(atlas__super.prototype);
...
function Atlas(def) {
  ...
  function make(def) {
    if (!def || is_fn.fn(def.draw)) {
      return def;
    }

    def = extend.extendjs({}, def);
  ...
  function find(query) {
    if (textures) {
      if (is_fn.fn(textures)) {
        return textures(query);
      } else if (is_hash.hash(textures)) {
        return textures[query];
      }
    }
    if (cutouts) { // deprecated
      ...
      for (var i = 0; i < cutouts.length; i++) {
        if (string(cutouts[i].name, query)) {
          if (n === 0) {
            result = cutouts[i];
          } else if (n === 1) {
...
function Selection(result, find, make) {
  function link(result, subquery) {
    ...
    } else if (is_fn.fn(result.draw)) {
      return result;

    } else if (is_hash.hash(result) && is_number.number(result.width)
        && is_number.number(result.height) && is_fn.fn(make)) {
      return make(result);

    } else if (is_hash.hash(result) && is_defined.defined(subquery)) {
      return link(result[subquery]);

    } else if (is_fn.fn(result)) {
      return link(result(subquery));

    } else if (is_array.array(result)) {
      return link(result[0]);

    } else if (is_string.string(result) && is_fn.fn(find)) {
  ...
  this.array = function(arr) {
    var array = is_array.array(arr) ? arr : [];
