...
_core.Class.atlas = function (def) {
  var atlas = _is.is_fn.fn(def.draw) ? def : new Atlas(def);
  if (def.name) {
  ...
  if (_is.is_string.string(def.image)) {
    url = def.image;
  } else if (_is.is_hash.hash(def.image)) {
    url = def.image.src || def.image.url;
    ratio = def.image.ratio || ratio;
  }
  url && _core.Class.preload(function (done) {
    url = _core.Class.resolve(url);
    DEBUG && console.log('Loading atlas: ' + url);
    var imageloader = _core.Class.config('image-loader');
...
atlas__super = _texture.Texture;
Atlas.prototype = (0, _create.createjs)(atlas__super.prototype);
...
function Atlas(def) {
  ...
  function make(def) {
    if (!def || _is.is_fn.fn(def.draw)) {
      return def;
    }

    def = (0, _extend.extendjs)({}, def);
  ...
  function find(query) {
    if (textures) {
      if (_is.is_fn.fn(textures)) {
        return textures(query);
      } else if (_is.is_hash.hash(textures)) {
        return textures[query];
      }
    }
    if (cutouts) {
      ...
      for (var i = 0; i < cutouts.length; i++) {
        if ((0, _string.startsWith)(cutouts[i].name, query)) {
          if (n === 0) {
            result = cutouts[i];
          } else if (n === 1) {
...
var nfTexture = new _texture.Texture();
...
function Selection(result, find, make) {
  function link(result, subquery) {
    if (!result) {
      return nfTexture;
    } else if (_is.is_fn.fn(result.draw)) {
      return result;
    } else if (_is.is_hash.hash(result) && _is.is_number.number(result.width) && _is.is_number.number(result.height) && _is.is_fn.fn(make)) {
      return make(result);
    } else if (_is.is_hash.hash(result) && _is.is_defined.defined(subquery)) {
      return link(result[subquery]);
    } else if (_is.is_fn.fn(result)) {
      return link(result(subquery));
    } else if (_is.is_array.array(result)) {
      return link(result[0]);
    } else if (_is.is_string.string(result) && _is.is_fn.fn(find)) {
  ...
  this.array = function (arr) {
    var array = _is.is_array.array(arr) ? arr : [];
    if (_is.is_array.array(result)) {
...
_core.Class.texture = function (query) {
  if (!_is.is_string.string(query)) {
