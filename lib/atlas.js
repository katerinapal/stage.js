...
import { startsWith as utilstring_startsWithjs } from "./util/string";
var DEBUG;
...
core_Classjs.atlas = function(def) {
  var atlas = utilis_isjs.fn(def.draw) ? def : new Atlas(def);
  if (def.name) {
  ...
  if (utilis_isjs.string(def.image)) {
    url = def.image;
  } else if (utilis_isjs.hash(def.image)) {
    url = def.image.src || def.image.url;
    ratio = def.image.ratio || ratio;
  }
  url && core_Classjs.preload(function(done) {
    url = core_Classjs.resolve(url);
    DEBUG && console.log('Loading atlas: ' + url);
    var imageloader = core_Classjs.config('image-loader');
...
Atlas._super = texture_Texturejs;
Atlas.prototype = utilcreate_createjsjs(Atlas._super.prototype);
...
function Atlas(def) {
  ...
  function make(def) {
    if (!def || utilis_isjs.fn(def.draw)) {
      return def;
    }

    def = utilextend_extendjsjs({}, def);

    if (utilis_isjs.fn(map)) {
      def = map(def);
    }
  ...
  function find(query) {
    if (textures) {
      if (utilis_isjs.fn(textures)) {
        return textures(query);
      } else if (utilis_isjs.hash(textures)) {
        return textures[query];
      }
    }
    if (cutouts) { // deprecated
      ...
      for (var i = 0; i < cutouts.length; i++) {
        if (utilstring_startsWithjs(cutouts[i].name, query)) {
          if (n === 0) {
            result = cutouts[i];
          } else if (n === 1) {
...
var nfTexture = new texture_Texturejs();
...
function Selection(result, find, make) {
  function link(result, subquery) {
    ...
    } else if (utilis_isjs.fn(result.draw)) {
      return result;

    } else if (utilis_isjs.hash(result) && utilis_isjs.number(result.width)
        && utilis_isjs.number(result.height) && utilis_isjs.fn(make)) {
      return make(result);

    } else if (utilis_isjs.hash(result) && utilis_isjs.defined(subquery)) {
      return link(result[subquery]);

    } else if (utilis_isjs.fn(result)) {
      return link(result(subquery));

    } else if (utilis_isjs.array(result)) {
      return link(result[0]);

    } else if (utilis_isjs.string(result) && utilis_isjs.fn(find)) {
  ...
  this.array = function(arr) {
    var array = utilis_isjs.array(arr) ? arr : [];
    if (utilis_isjs.array(result)) {
...
core_Classjs.texture = function(query) {
  if (!utilis_isjs.string(query)) {
