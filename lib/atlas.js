...
_core.Class.atlas = function (def) {
  var atlas = (0, _is.is_fn)(def.draw) ? def : new Atlas(def);
  if (def.name) {
  ...
  if ((0, _is.is_string)(def.image)) {
    url = def.image;
  } else if ((0, _is.is_hash)(def.image)) {
...
function Atlas(def) {
  ...
  function find(query) {
    if (textures) {
      if ((0, _is.is_fn)(textures)) {
        return textures(query);
      } else if ((0, _is.is_hash)(textures)) {
        return textures[query];
      }
    }
    if (cutouts) {
      ...
      for (var i = 0; i < cutouts.length; i++) {
        if (_string.startsWith.startsWith(cutouts[i].name, query)) {
          if (n === 0) {
            result = cutouts[i];
          } else if (n === 1) {
...
function Selection(result, find, make) {
  function link(result, subquery) {
    if (!result) {
      return nfTexture;
    } else if ((0, _is.is_fn)(result.draw)) {
      return result;
    } else if ((0, _is.is_hash)(result) && (0, _is.is_number)(result.width) && (0, _is.is_number)(result.height) && (0, _is.is_fn)(make)) {
      return make(result);
    } else if ((0, _is.is_hash)(result) && (0, _is.is_defined)(subquery)) {
      return link(result[subquery]);
    } else if ((0, _is.is_fn)(result)) {
      return link(result(subquery));
    } else if ((0, _is.is_array)(result)) {
      return link(result[0]);
    } else if ((0, _is.is_string)(result) && (0, _is.is_fn)(find)) {
  ...
  this.array = function (arr) {
    var array = (0, _is.is_array)(arr) ? arr : [];
    if ((0, _is.is_array)(result)) {
