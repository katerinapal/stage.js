...
ClassBinding.atlas = function (def) {
  ...
  url && ClassBinding.preload(function (done) {
    url = ClassBinding.resolve(url);
    DEBUG && console.log('Loading atlas: ' + url);
    var imageloader = ClassBinding.config('image-loader');
...
Atlas._super = _texture.Texture;
Atlas.prototype = (0, _create.createjs)(Atlas._super.prototype);
...
ClassBinding.texture = function (query) {
  if (!is.string(query)) {
