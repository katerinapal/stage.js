...
import { repeatjs as repeat } from "./util/repeat";
...
image__super = Class;
Image.prototype = create.createjs(image__super.prototype);
...
Image.prototype._repeat = function(stretch, inner) {
  ...
  this.tick(this._repeatTicker = function() {
    ...
    this._textures.length = repeat.repeatjs(this._image, width, height, stretch, inner,
        insert);
  });
  ...
  function insert(i, sx, sy, sw, sh, dx, dy, dw, dh) {
    ...
    repeat.repeatjs.src(sx, sy, sw, sh);
    repeat.repeatjs.dest(dx, dy, dw, dh);
  }
};
var exported_Image = Image;
export { exported_Image as Image };
