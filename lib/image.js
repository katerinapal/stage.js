...
import { repeatjs as utilrepeat_repeatjsjs } from "./util/repeat";
import { createjs as utilcreate_createjsjs } from "./util/create";
import "./pin";
import "./loop";

core_Classjs.image = function(image) {
  var img = new Image();
  image && img.image(image);
  return img;
};

Image._super = core_Classjs;
Image.prototype = utilcreate_createjsjs(Image._super.prototype);
...
Image.prototype.image = function(image) {
  this._image = core_Classjs.texture(image).one();
...
Image.prototype._repeat = function(stretch, inner) {
  ...
  this.tick(this._repeatTicker = function() {
    ...
    this._textures.length = utilrepeat_repeatjsjs(this._image, width, height, stretch, inner,
        insert);
  });
