...
it('visit', function() {
  ...
  expect.expect(visitor.start.args.pluck(0)).list(
      node([ 1, 11, 12, 121, 122, 123, 13 ]), 'id');
  expect.expect(visitor.start.alwaysCalledWithMatch(sinon.match.object, data)).ok();
  ...
  expect.expect(visitor.start.args.pluck(0)).list(node([ 1, 11, 12, 122, 123, 13 ]),
      'id');
  expect.expect(visitor.start.alwaysCalledWithMatch(sinon.match.object, data)).ok();
  expect.expect(visitor.start.alwaysCalledOn(visitor)).ok();
